...
def _hotel_nights(hotel: dict) -> int:
    checkin = _date_obj(hotel.get("date", ""))
    checkout = _date_obj(hotel.get("checkout", ""))
...
def _trip_dates(trip: dict) -> list[date]:
    ...
    for item in _timeline_items(trip):
        ...
        if item.get("checkout"):
            co = _date_obj(item.get("checkout", ""))
            if co:
                dates.append(co)
    return sorted(dates)
...
def _total_nights(all_trips: list[dict] | None) -> int:
    return sum(_hotel_nights(h) for t in (all_trips or []) for h in t.get("hotels", []))


def _detect_prefecture(place: str, settings: dict | None = None) -> str | None:
...
def _collect_places(trip: dict) -> list[str]:
    places = []
    for p in trip.get("plans", []):
        if p.get("place"):
...
def _all_travel_stats(all_trips: list[dict] | None, settings: dict | None = None) -> dict:
    ...
    for t in all_trips:
        for place in _collect_places(t):
            pref = _detect_prefecture(place, settings)
            if pref:
                prefs.add(pref)
    ...
    return {
        ...
        "prefectures": ordered,
        "nights": _total_nights(all_trips),
    }
...
def _make_styles():
    ...
    styles.add(ParagraphStyle(name="CoverTitle", parent=styles["Title"], fontName="HeiseiKakuGo-W5", fontSize=30, leading=38, textColor=BROWN, alignment=1, spaceAfter=8))
    styles.add(ParagraphStyle(name="CoverSub", parent=styles["Normal"], fontName="HeiseiKakuGo-W5", fontSize=12, leading=18, textColor=MUTED, alignment=1))
    styles.add(ParagraphStyle(name="SectionTitle", parent=styles["Heading2"], fontName="HeiseiKakuGo-W5", fontSize=16, leading=22, textColor=BROWN, spaceBefore=8, spaceAfter=8))
    styles.add(ParagraphStyle(name="CardTitle", parent=styles["Heading3"], fontName="HeiseiKakuGo-W5", fontSize=11, leading=15, textColor=BROWN, alignment=1))
    styles.add(ParagraphStyle(name="Small", parent=styles["Normal"], fontName="HeiseiKakuGo-W5", fontSize=8, leading=11, textColor=DARK))
    styles.add(ParagraphStyle(name="TinyMuted", parent=styles["Normal"], fontName="HeiseiKakuGo-W5", fontSize=7, leading=9, textColor=MUTED))
    styles.add(ParagraphStyle(name="DayHeader", parent=styles["Normal"], fontName="HeiseiKakuGo-W5", fontSize=12, leading=16, textColor=BROWN, alignment=1))
    styles.add(ParagraphStyle(name="BoardCell", parent=styles["Normal"], fontName="HeiseiKakuGo-W5", fontSize=8, leading=10, textColor=DARK))
    return styles
...
def _cover_page(story, trip: dict, settings: dict, styles, all_trips: list[dict] | None = None):
    ...
    for h in trip.get("hotels", [])[:8]:
        ...
        if h.get("checkout"):
            line += f" → {h.get('checkout')}（{_hotel_nights(h)}泊）"
    ...
    if stats.get("prefectures"):
        story.append(_card_table("行った都道府県", ["・" + "、".join(stats["prefectures"])], styles, width=725))
        story.append(Spacer(1, 10))
    ...
    for chunk_index, chunk in enumerate(chunks):
            ...
            for d in chunk:
                if row_i < len(grouped[d]):
                    ...
                    if item["type"] == "hotel":
                        ...
                        if item.get("checkout"):
                            text += f"<br/>OUT：{item.get('checkout')}（{item.get('nights', 1)}泊）"
                    else:
                        text = f"<b>{item.get('time','')}</b>　{item.get('title','')}　[{item.get('category','')}]"
                        if item.get("place"):
                            text += f"<br/>場所：{item.get('place')}"
        ...
        for col_i, d in enumerate(chunk):
            for row_i, item in enumerate(grouped[d], start=1):
                style_cmds.append(("BACKGROUND", (col_i, row_i), (col_i, row_i), HOTEL_BG if item["type"] == "hotel" else PLAN_BG))
...
def _details_pages(story, trip: dict, settings: dict, styles):
    ...
    packing_lines = []
    for item in trip.get("packing", [])[:30]:
