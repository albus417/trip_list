...
def get_supabase_client():
    ...
    try:
        ...
        st.error(
            "Supabase の Secrets ファイルが見つかりません。"
            "ローカルでは .streamlit/secrets.toml を作成してください。"
        )
        st.stop()
...
def normalize_email(email: str) -> str:
    """メールアドレスの前後の空白や大文字を整える。"""
    return (email or "").strip().lower()


def auth_error_message(error: Exception) -> str:
    """Supabaseのエラーを利用者向けに少し分かりやすくする。"""
    raw = str(error)
    lower = raw.lower()

    if "email rate limit exceeded" in lower:
        return "短時間に登録メールを送りすぎたため、一時的に制限されています。30分〜1時間ほど待ってから再試行してください。開発中はSupabase側でメール確認をOFFにすると試しやすいです。"
    ...
    if "password" in lower and ("6" in lower or "short" in lower):
...
def save_auth_session(session, user):
    """SupabaseのセッションをStreamlit側に保存する。"""
...
def ensure_user_data():
    ...
    try:
        ...
        st.error(f"Supabase の app_data を準備できませんでした：{auth_error_message(e)}")
        st.stop()
        ...
        if submitted:
            ...
            if not email or not password:
                ...
                try:
                    ...
                    st.error(f"ログインできませんでした：{auth_error_message(e)}")

    with tab_signup:
        st.info("初めて使う方はこちら。旅行データはユーザーごとに分かれて保存されます。")
        with st.form("signup_form", clear_on_submit=False):
            ...
            agree = st.checkbox("このアプリでは自分の旅行データだけが保存・表示されることを理解しました。")
        ...
        if submitted:
            ...
            if not email:
                ...
                st.warning("メールアドレスに空白が入っています。")
            elif len(password) < 6:
                st.warning("パスワードは6文字以上にしてください。")
            elif password != password_confirm:
                st.warning("確認用パスワードが一致していません。")
            elif not agree:
                st.warning("確認項目にチェックを入れてください。")
            else:
                try:
                    ...
                    # Supabase側でメール確認OFFなら session が返ってそのままログイン可能。
                    ...
                    st.error(f"登録できませんでした：{auth_error_message(e)}")

    with tab_reset:
        st.info("パスワードを忘れた場合、再設定メールを送信します。")
        with st.form("reset_password_form", clear_on_submit=False):
            reset_email = st.text_input("登録したメールアドレス", key="reset_email")
        ...
        if submitted:
            ...
            if not reset_email:
                ...
                try:
                    ...
                    st.error(f"再設定メールを送れませんでした：{auth_error_message(e)}")

    st.caption("※ 開発中に email rate limit exceeded が出る場合は、Supabaseのメール送信制限です。時間を置くか、SupabaseのEmail設定でメール確認をOFFにしてください。")
    return False
